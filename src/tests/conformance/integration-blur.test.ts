...
describe('Conformance: Integration - Precomputed Blur', () => {
  ...
  backends.forEach(backend => {
    it(`should execute the Precomputed Blur pipeline (Resize -> Gen -> Blur) [${backend.name}]`, async () => {
      ...
      // Mock inputs to satisfy the new IR
      inputs.set('t_input', { width: 4, height: 4, data: [new Float32Array(4 * 4 * 4)] } as any);

      // We expect this to fail compilation on Metal
      try {
